...
def require_api_key(func: Callable[..., Response]) -> Callable[..., Response]:
...
@require_api_key
def execute_plugin() -> tuple[Response, int]:
    try:
        ...
        plugin_info = manager.registry.get_plugin(command)
        ...
        return jsonify({
            ...
            "plugin": plugin_info.name if plugin_info else None,
...
@require_api_key
def plugin_help() -> tuple[Response, int]:
    try:
...
@require_api_key
def get_api_user() -> tuple[Response, int]:
