...
from plugins.base.manager import PluginContext
from services.chat_service import get_active_users, get_chat_profile_data, get_conversations
...
def handle_send_message(data: dict) -> None:
    ...
    if content.startswith("/"):
        ...
        if plugin_mgr:
            ...
            try:
                response = plugin_mgr.execute_command(content, ctx)
...
def handle_request_edit(data: dict) -> None:
    ...
    if msg and msg.user_id == current_user.id:
        emit("edit_allowed", {"message_id": message_id, "content": msg.content})


@socketio.on("request_delete")
def handle_request_delete(data: dict) -> None:
    message_id = data.get("message_id")
    if not current_user.is_authenticated:
        return
...
@login_required
def api_edit_message(message_id: int) -> tuple[object, int]:
    data = request.get_json()
    content = data.get("content", "").strip()

    message, error = _edit_message(message_id, current_user.id, content)
    if error:
        return jsonify({"error": error}), 403

    socketio.emit("message_edited", {"message_id": message.id, "content": message.content})
    return jsonify({"success": True})


@chat_bp.route("/message/<int:message_id>/delete", methods=["POST"])
@login_required
def api_delete_message(message_id: int) -> tuple[object, int]:
    success, error = _delete_message(message_id, current_user.id)
    if not success:
        return jsonify({"error": error}), 403
...
@login_required
def upload_image() -> tuple[object, int]:
